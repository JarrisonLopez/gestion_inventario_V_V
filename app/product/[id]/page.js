...
export default function ProductPage({ params: asyncParams }) {
  ...
  useEffect(() => {
    ...
    // Resuelve los parámetros asincrónicos
    (async () => {
      ...
      try {
        ...
        if (!res.ok) {
          if (res.status === 404) {
            ...
            setError('Error al cargar el producto. Por favor, inténtelo más tarde.');
          }
          return;
        }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-blue-50 to-blue-100">
        <div className="w-full max-w-lg p-6 bg-white rounded-3xl shadow-xl text-center">
          ...
          <Link href="/product" className="mt-6 inline-block px-4 py-2 bg-blue-600 text-white font-semibold rounded-lg shadow-md hover:bg-blue-700 transition duration-200">
              Atrás
          </Link>
        </div>
      </div>
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-blue-50 to-blue-100">
      <div className="w-full max-w-lg p-6 bg-white rounded-3xl shadow-xl">
        ...
        <p className="text-lg font-semibold text-gray-800 mb-2">
          Precio: <span className="text-blue-600">${product.price}</span>
        </p>
        <p className="text-lg font-semibold text-gray-800">
          Stock: <span className="text-blue-600">{product.stock}</span>
        </p>
        <div className="flex space-x-4 mt-6">
          {/* Mostrar los botones de Editar y Eliminar solo si es admin */}
          {isAdmin && (
            <>
              <Link href={`/edit-product/${params.id}`} className="px-4 py-2 bg-yellow-500 text-white font-semibold rounded-lg shadow-md hover:bg-yellow-600 transition duration-200">
                Editar
              </Link>
              <button
          ...
          {/* Botón Atrás disponible para todos */}
          <Link href="/product" className="px-4 py-2 bg-blue-600 text-white font-semibold rounded-lg shadow-md hover:bg-blue-700 transition duration-200">
              Atrás
          </Link>
        </div>
      </div>
